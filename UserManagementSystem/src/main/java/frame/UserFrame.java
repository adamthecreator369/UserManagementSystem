			...
			btn.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (sender.equals(layout1.getTopRtBtn1())) {
						...
						currentUser = newUser;
						// log in new user
						logList.add(new LogEvent(currentUser.getUsername(), "LOGIN", "current time"));
						// set current user text to new user.
						mainLayout.getCurrentUserLabel().setText(R.string.logged_in_as + currentUser.getName());
